...
struct TestFixture
{
  TestFixture()
  {
    ...
    reactor->init();
    engine = reactor->getEngine<Engine>("Avi", NULL);
  }

  ~TestFixture()
  {
    BOOST_TEST_MESSAGE("Stopping and destroying Reactor ");
    engine.reset();
    reactor.reset();
  }
};
...
BOOST_AUTO_TEST_SUITE(engine_tests, *boost::unit_test::fixture<TestFixture>())
  ...
  if (stationQueryData.itsValues.size() == 1)
  {
    ...
    if (valuesIt->second.size() == 9)
    {
           ...
           ++qvIt)
      {
        ...
        if (name == "messageid")
        {
          std::string value = vvIt->apply_visitor(sv);
        ...
        else if (name == "messagevalidfrom")
        {
          ...
          //
          // BOOST_CHECK_THROW(vvIt->apply_visitor(sv), Fmi::Exception);
        }
    ...
    for (const auto &id : stationIdList)
    {
           ...
           ++qvIt)
      {
        ...
        if (name == "messagetype")
        {
          ...
          {
            std::string valueStr = value.apply_visitor(sv);
